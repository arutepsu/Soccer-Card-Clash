...
import scalafx.scene.layout.{HBox, VBox, StackPane, Region}
import view.components.gameComponents._
...
case class GamePlayerScene(
  ...
  // ✅ Button Actions (Right-Side Panel)
  val attackButton: Button = GameButtonFactory.createGameButton("Attack", 150, 50) { () =>
  ...
  val undoButton: Button = GameButtonFactory.createGameButton("Undo", 150, 50) { () =>
    controller.undo()
    updateDisplay()
    gameStatusBar.updateStatus(GameStatusMessages.UNDO_PERFORMED)
  }

  val redoButton: Button = GameButtonFactory.createGameButton("Redo", 150, 50) { () =>
    controller.redo()
    updateDisplay()
    gameStatusBar.updateStatus(GameStatusMessages.REDO_PERFORMED)
  }

  val showDefendersButton: Button = GameButtonFactory.createGameButton("Show Defenders", 180, 50) { () =>
  ...
  val makeSwapButton: Button = GameButtonFactory.createGameButton("Make Swap", 180, 50) { () =>
  ...
  // ✅ Group Buttons into a VBox (Right-Side)
  val actionButtons = new VBox {
    alignment = Pos.TOP_RIGHT
    spacing = 15
    padding = Insets(20)
    children = Seq(attackButton, undoButton, redoButton, showDefendersButton, makeSwapButton, mainMenuButton)
  }

  // ✅ Top Section: Score + Game Status
  val topSection = new VBox {
    alignment = Pos.TOP_CENTER
    spacing = 5
    children = Seq(gameStatusBar, new HBox {
      alignment = Pos.CENTER
      spacing = 50
      children = Seq(player1ScoreLabel, player2ScoreLabel)
    })
  }

  // ✅ Middle Section: Field & Actions
  val middleSection = new HBox {
    alignment = Pos.CENTER
    spacing = 100
    children = Seq(defenderFieldBar, actionButtons)
  }

  // ✅ Bottom Section: Attacker’s Hand
  val bottomSection = new VBox {
  ...
  root = new StackPane {
    children = Seq(
      ...
      new VBox {
        ...
        alignment = Pos.CENTER
        children = Seq(
          topSection,     // ✅ Scores & Status
          middleSection,  // ✅ Field & Actions
          bottomSection   // ✅ Attacker’s Hand
        )
      }
    )
  }
  ...
  def updateDisplay(): Unit = {
    ...
    attackerHandBar.updateHand()
    defenderFieldBar.updateField()
