...
class FieldState(val playingField: PlayingField, val player1: Player, val player2: Player) {
  ...
  private var player1Cards: mutable.Queue[Card] = mutable.Queue()
  private var player2Cards: mutable.Queue[Card] = mutable.Queue()

  /** 📌 Initializes player hands */
  def initializePlayerHands(player1Hand: List[Card], player2Hand: List[Card]): Unit = {
    player1Cards = player1Hand.to(mutable.Queue)
    player2Cards = player2Hand.to(mutable.Queue)
  }
