package de.htwg.se.soccercardclash.model.gameComponent.action.strategy.trigger.attack.base
...
class DoubleAttackStrategySpec extends AnyWordSpec with Matchers {
  ...
  class TestableDoubleAttackStrategy(
                                      ...
                                      revertBoostStrategy: IRevertBoostStrategy
                                    ) extends DoubleAttackStrategy(index, playerActionService, revertBoostStrategy) {
  ...
  "processDefenderAttack" should {
    "return updated state when attacker wins in double attack" in {
      ...
      val revertStrategy = mock(classOf[IRevertBoostStrategy])
      ...
      val strategy = new TestableDoubleAttackStrategy(0, dummyPlayerActionManager, revertStrategy)
  ...
  "processGoalkeeperAttack" should {
    "return updated state when attacker beats goalkeeper" in {
      ...
      val strategy = new TestableDoubleAttackStrategy2(0, dummyActionManager, revertStrategy )
  ...
  "attackerWins" should {
    "add all non-empty cards to hand and return updated game state with success event" in {
      ...
      val dummyActionManager = mock(classOf[IPlayerActionManager])
      val strategy = new TestableDoubleAttackStrategy3(0, dummyActionManager, revertStrategy)
  ...
  "handleTie" should {
    "resolve tie by comparing extra cards, attacker wins" in {
      ...
      val dummyActionManager = mock(classOf[IPlayerActionManager])
      val strategy = new TestableDoubleAttackStrategy4(0, dummyActionManager, revertStrategy)
      ...
      events.exists(_ == resultEvent) shouldBe false
