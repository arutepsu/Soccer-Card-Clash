package de.htwg.se.soccercardclash.model.gameComponent.action.strategy.trigger.swap.base
...
import de.htwg.se.soccercardclash.model.gameComponent.components.{IGameCards, Roles}
...
class RegularSwapStrategySpec extends AnyWordSpec with Matchers with MockitoSugar {
  ...
  "RegularSwapStrategy" should {
    ...
    "perform a regular swap successfully" in {
      ...
      val (success, result, events) = strategy.execute(state)
    ...
    "fail if swap index is invalid" in {
      ...
      val (success, _, events) = strategy.execute(state)
